...
export function buildPreview(pageTitle: string | undefined, links: DetectedLink[], folderName?: string): PreviewResponse {
...
export function buildJobsFromBatch(batch: CreateBatchRequest): Array<{
  ...
  const originalByUrl = new Map<string, (typeof selectedInput)[number]>();
...
function resolveFolderMatch(title: string, year: string | undefined, mediaType: MediaType, preferredFolderName?: string): {
  title?: string;
  year?: string;
...
function listLibraryFolders(mediaType: MediaType): Array<{ name: string; title: string; year?: string }> {
...
function parseFolderTitle(value: string): { title: string; year?: string } {
...
function titleFromUrl(rawUrl: string): string | undefined {
  try {
