...
import type { CreateBatchRequest, DetectedLink, MediaType } from "./types.js";
...
type WsSocket = { send: (data: string) => void; on: (event: string, cb: (data?: unknown) => void) => void };
...
function registerApiRoutes(prefix: string): void {
  ...
  app.post(`${prefix}/api/download-batches`, async (request, reply) => {
    const body = request.body as Partial<CreateBatchRequest>;
