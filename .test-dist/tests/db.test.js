...
test("database startup reports unwritable SQLite directories clearly", { skip: process.platform === "win32" || process.getuid?.() === 0 }, () => {
    const root = fs.mkdtempSync(path.join("/tmp", "cnama-plex-db-unwritable-"));
    const dbPath = path.join(root, "cnama.sqlite");
    fs.chmodSync(root, 0o555);
    try {
        assert.throws(() => new AppDatabase(dbPath), /SQLite database is not writable at .*cnama\.sqlite.*PUID\/PGID/);
    }
    finally {
